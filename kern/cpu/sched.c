...
{
	{
		...
		if (quantum == 0)
		    panic("Quantum cannot be zero for PRIRR scheduler");

		if((int)numOfPriorities >0)
			{
				num_of_ready_queues = numOfPriorities;

			}
			else if((int)numOfPriorities == 0)
			{
				sched_init_RR(quantum);
				return;
			}
			else{
				panic("Can't have negative number of queues");
			}

	sched_delete_ready_queues(); //to remove already made ones
...
struct Env* fos_scheduler_PRIRR()
{
	...
	struct Env* enew = NULL;
	struct Env* elast = get_cpu_proc();
	//check to re enqueue current process on cpu to put it back to the right queue
	if(elast != NULL && elast->env_status == ENV_READY)
	{
		//elast->env_status = ENV_READY;
		enqueue(&(ProcessQueues.env_ready_queues[elast->priority]), elast);
	}
	...
	for(int i=0;i<num_of_ready_queues;i++)
	{
		...
		{
			enew = dequeue(&(ProcessQueues.env_ready_queues[i]));
			break;
		}
	}
	if (enew != NULL)
	{
		// reset starvation counter for the selected env
		enew->starvcounteer = 0;
		kclock_set_quantum(quantums[0]);
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	if (isSchedMethodPRIRR())
	{
		...
		for(int i=0;i<num_of_ready_queues;i++)
		{
			struct Env* efirst = ProcessQueues.env_ready_queues[i].lh_first;
			if(ProcessQueues.env_ready_queues[i].size == 0)
			{
				continue;
			}
			struct Env* curenv=NULL;
			LIST_FOREACH_SAFE(curenv, &ProcessQueues.env_ready_queues[i], Env)
			{
				curenv->starvcounteer++;
				if(curenv->starvcounteer > starvThreshold)
				{
					int newprio= curenv->priority -1;
					if(newprio < 0)
					{
						continue;
					}

					env_set_priority(curenv->env_id, newprio);
...
void update_WS_time_stamps()
{
	...
	//Your code is here
	struct Env* cur_env = get_cpu_proc();
	    if (cur_env == NULL) return;

	    struct WorkingSetElement *ws_iter;
	    LIST_FOREACH(ws_iter, &(cur_env->page_WS_list))
	    {
	        // Get current permissions to check USED bit
	        uint32 perms = pt_get_page_permissions(cur_env->env_page_directory,
	                                              ws_iter->virtual_address);

	        // Shift counter right by 1 bit
	        ws_iter->time_stamp >>= 1;

	        // Add USED bit to the leftmost position
	        if (perms & PERM_USED) {
	            ws_iter->time_stamp |= (1 << 31); // Set MSB to 1
	        }

	        // Clear the USED bit
	        pt_set_page_permissions(cur_env->env_page_directory,
	                               ws_iter->virtual_address, 0, PERM_USED);
	    }
	//Comment the following line
