...
extern uint32 starvThreshold;
void env_set_priority(int envID, int priority)
{
#if USE_KHEAP
	acquire_kspinlock(&ProcessQueues.qlock);
	struct Env* env;
	//get pointer to env from the envid
	int flag = envid2env(envID, &env, 0);

	// env not null
	if(flag == 0)
	{
		// make sure that the new priority is valid for the ready queue array
		if (priority < 0 || priority >= num_of_ready_queues) {
			release_kspinlock(&ProcessQueues.qlock);
			panic("priority out of bounds for the ready queue array.");
		}

		// if status of env is ready remove it from the old ready queue and insert it in the queue with the new priority
		if(env->env_status == ENV_READY)
		{
			sched_remove_ready(env);

			env->priority = priority;
			//since it's removed from old queue make the starvation counter =0 again
			env->starvcounteer = 0;
		...
		{
			// If not ready, just update the priority
			env->priority = priority;
		}
	}
	else
...
#if USE_KHEAP
	if(starvThresh <= 0)
		panic("can't have threshold less than or equal to zero");
