...
void env_init(void)
{
	init_kspinlock(&env_lock, "env_lock");
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
...
#if USE_KHEAP
	//acquire lock to protect heap
	acquire_kspinlock(&env_lock);

	//save the start of the saved space
	void *kstack = kmalloc(KERNEL_STACK_SIZE);

	if (kstack == NULL) {
		panic("No memory in heap for styack");
	}

	uint32 *page_table_ptr = NULL;
	//get the page table of the page that contains the stack pointer
	get_page_table(ptr_user_page_directory, (uint32)kstack, &page_table_ptr);

	if (page_table_ptr != NULL) {
		//make the present bit = 0, to make sure the guard page isn't accessable
		page_table_ptr[PTX(kstack)] = page_table_ptr[PTX(kstack)] & (~PERM_PRESENT);
	}
	release_kspinlock(&env_lock);
	...
	uint32 kernstack_top = KERN_STACK_TOP - env_count * (KERNEL_STACK_SIZE);

	//get number of frames needed for the kernel
	int numberOfFrames = KERNEL_STACK_SIZE / PAGE_SIZE;
	for(int i=0;i<numberOfFrames-1;i++)
	{
		// allocate a frame from physical mempory then map it in virtual memory using page directory pointer
		struct FrameInfo* frame = NULL;
		alloc_page(ptr_page_directory,kernstack_top - ((i+1)*PAGE_SIZE),PERM_WRITEABLE | PERM_PRESENT,0);
	}
	void* ptr =  (void*)(kernstack_top - KERNEL_STACK_SIZE);
	env_count++;
