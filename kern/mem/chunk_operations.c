...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//ezbot el limitts bas lazem tkoon nearest 4KB
    uint32 start = ROUNDDOWN(virtual_address, PAGE_SIZE);
    uint32 end   = ROUNDUP(virtual_address + size, PAGE_SIZE);

    // da el page directory el OS byesta3mlo 3ashan y3dl fel address space bta3 el process
        uint32 *page_dir = e->env_page_directory;

    for (uint32 cur = start; cur < end; cur += PAGE_SIZE)
       {
    	 uint32 *page_table = NULL;

    	        // geeb el page table law mawgoda
    	        int res = get_page_table(page_dir, cur, &page_table);

    	        // hena law TABLE_NOT_EXIST aw page_table == NULL ben3mel create_page_table
    	        if ( page_table == NULL)
    	        {
    	            page_table = (uint32 *) create_page_table(page_dir, cur);
    	        }

    	        // el index gowa el page table
     	        uint32 entry_index = PTX(cur);

    	        page_table[entry_index] = 0;

     	        page_table[entry_index] = PERM_UHPAGE;
       }
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
    ...
    uint32 va_end   = ROUNDUP(virtual_address + size, PAGE_SIZE);
     ...
     for (uint32 v = va_start; v < va_end; v += PAGE_SIZE)
    {
        // unmark
        pt_set_page_permissions(e->env_page_directory, v, 0, PERM_UHPAGE);

        // sheel men el page file
        pf_remove_env_page(e, v);

        // sheel men el working set
