...
//	uint32 *ptr;
//	get_page_table(directory , virtual_address, &ptr);
//	ptr[PTX(virtual_address)] >> 22;
	uint32 *page_table = NULL;

	    // Try to get the page table for this virtual address
	    get_page_table(directory, virtual_address, &page_table);

	    // If the page table doesn’t exist, return -1
	    if (page_table == NULL)
	        return -1;

	    // Get the entry for the specific page
	    uint32 page_entry = page_table[PTX(virtual_address)];

	    // If the entry is not present, you can still return its permissions,
	    // but usually we just return the flags bits (lower 12 bits)
	    return page_entry & 0x00000FFF; // Mask the flags (permissions bits)
